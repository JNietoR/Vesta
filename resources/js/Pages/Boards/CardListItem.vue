...
<template>
    <li>
        <div class="group relative bg-white shadow rounded-md border-b border-gray-300 hover:bg-gray-50">
            <form class="p-2.5" v-if="isShowingForm" @keydown.esc="isShowingForm = false" @submit.prevent="onSubmit()">
                <textarea ref="inputTitleRef" v-model="form.title"
                    class="block w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-blue-400 focus:ring-blue-400"
                    placeholder="Enter card title..." rows="3" @keydown.enter.prevent="onSubmit()"></textarea>
            ...
            <template v-if="!isShowingForm">
                <Link class="text-sm block p-2.5" :href="route('boards.show', { board: card.board_id, card: card.id })"
                    preserve-state>
                {{ card.title }}
                </Link>
